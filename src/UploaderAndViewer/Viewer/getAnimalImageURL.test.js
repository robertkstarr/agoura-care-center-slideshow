...
test('returns thumbnail with filename and animalId', async () => {
    expect(await getAnimalImageURL({ fileName: 'test.jpg', animalId: '1' }, true)).toEqual(
        MOCK_THUMBNAIL_URL
    );
});

test('returns image with filename and animalId', async () => {
    expect(await getAnimalImageURL({ fileName: 'test.jpg', animalId: '1' }, false)).toEqual(
        MOCK_URL
    );
});
...
test('returns image with filename and animalId and image url', async () => {
    expect(
        await getAnimalImageURL({ fileName: 'test.jpg', animalId: '1', url: SAVED_URL })
    ).toEqual(MOCK_URL);
});

test('returns image url with only image url', async () => {
    expect(await getAnimalImageURL({ url: SAVED_URL })).toEqual(SAVED_URL);
});

test('returns image url with filename and image url', async () => {
    expect(await getAnimalImageURL({ fileName: 'test.jpg', url: SAVED_URL })).toEqual(SAVED_URL);
});

test('returns image url with animal id and image url', async () => {
    expect(await getAnimalImageURL({ animalId: '1', url: SAVED_URL })).toEqual(SAVED_URL);
});
